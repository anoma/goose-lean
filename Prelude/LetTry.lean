...
syntax withPosition("let" "try" term (":" term)? ":=" term) "failwith" term optSemicolon(doSeq) : doElem
syntax withPosition("let" "try" term (":" term)? "←" term) "failwith" term optSemicolon(doSeq) : doElem
...
/-- The `let try x := ov; b` macro unwraps an `Option`, for `ov = some v` binds
  ...
  `let try ← mov; b` can be used when `mov : m (Option V)` for some monad `m`.
  The binder `x` can be given a type and it can be an arbitrary match pattern. -/
macro_rules
...
| `(let try $x:term := $e:term failwith $r ; $body) =>
  `(match ($e) with | none => $r | some $x => $body)
| `(let try $x:term : $t:term := $e:term failwith $r ; $body) =>
  `(match ($e) with | none => $r | some ($x : $t) => $body)
| `(doElem| let try $x:term := $e:term ; $body) =>
  `(doElem| match ($e) with | none => pure default | some $x => $body)
| `(doElem| let try $x:term : $t:term := $e:term ; $body) =>
  `(doElem| match ($e) with | none => pure default | some ($x : $t) => $body)
| `(doElem| let try $x:term := $e:term failwith $r ; $body) =>
  `(doElem| match ($e) with | none => pure $r | some $x => $body)
| `(doElem| let try $x:term : $t:term := $e:term failwith $r ; $body) =>
  `(doElem| match ($e) with | none => pure $r | some ($x : $t) => $body)
| `(doElem| let try $x:term ← $e:term ; $body) =>
  `(doElem| match ← $e with | none => pure default | some $x => $body)
| `(doElem| let try $x:term : $t:term ← $e:term ; $body) =>
  `(doElem| match ← $e with | none => pure default | some ($x : $t) => $body)
